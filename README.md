...
- 🔍 画布缩放 / 平移，画笔精确跟随指针
- 🔗 二维码配对，扫码即连
- 🛡️ 断线保护：网络中断自动重连 + 断线期间操作补同步
- 🌐 跨网兜底：TURN 中继穿透对称 NAT（默认不携带凭据，需自配）
...
默认**不携带**任何私有 TURN 凭据（仅公共测试服务器作 fallback，通常不可用）。跨网络连接需配置自己的 TURN 服务器，例如自部署 coturn：

1. 桌面端：应用设置面板添加，或编辑 `~/.juanlink/turn.json`：
...
3. 跨网连接时，Initiator 会优先尝试自定义服务器，成功者写进二维码，Responder 据此连同一台。
