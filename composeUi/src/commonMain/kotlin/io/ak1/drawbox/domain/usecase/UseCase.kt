...
class UseCase {
    ...
    /**
     * 笔画级橡皮（对齐旧版矢量遮罩裁剪语义）：
     * - [Element.Path]：擦除盘内 sample 移除，断点处剩余样本拆分为多段 Path
     *   保留（主段沿用原 id，其余段新 id）——整条路径完全擦除时删除该元素，
     *   部分擦除时剩余墨线不消失，只裁剪相交段。
     * - 其他元素（Shape/Image/Text）无法笔画级裁剪，仍按 [hitTest] 整元素删除。
     * 未命中时返回原列表实例，供调用方引用比较短路（Reducer 的 EraseAt 依赖）。
     */
    fun eraseAt(
        elements: List<Element>,
        point: Offset,
        ...
        var mutated = false
        val result = ArrayList<Element>(elements.size + 2)
        for (el in elements) {
            if (el is Element.Path) {
                val pieces = splitPathByErase(el, point, radius)
                if (pieces === null) {
                    result.add(el)
                } else {
                    mutated = true
            ...
            } else if (el.hitTest(point, radius)) {
                mutated = true
            } else {
                result.add(el)
            }
        }
        return if (mutated) result else elements
    }

    /**
     * 将一条路径按擦除盘裁剪为若干段。返回 null = 完全未被擦到（保持原引用，
     * 调用方可引用比较短路）；空列表 = 整条路径被擦除；否则为分割后的段列表。
     * 判定：sample.position 距擦除中心 ≤ radius + sample.width/2（stroke 半宽
     * 计入），与 hitTestPath 的容差语义一致。
     */
    @OptIn(ExperimentalTime::class, ExperimentalUuidApi::class)
    private fun splitPathByErase(
        path: Element.Path,
        point: Offset,
        radius: Float,
    ): List<Element>? {
        ...
        val hit = BooleanArray(samples.size)
        var anyHit = false
        for (i in samples.indices) {
            val r = radius + samples[i].width / 2f
            val dx = samples[i].position.x - point.x
            val dy = samples[i].position.y - point.y
            if (dx * dx + dy * dy <= r * r) {
                hit[i] = true
                anyHit = true
            }
        ...
        // 连续未擦样本分组为段；被擦样本断开。主段（含首个保留样本）沿用原 id，
        // 其余段生成新 id——dispatchLocalOps 的 diff 天然产生「原 id 内容变化 +
        // 新段插入」，跨端同步与 undo 逆 op（各段 upsert 逆 = remove）都成立。
        val now = Clock.System.now().toEpochMilliseconds()
        val pieces = ArrayList<Element>()
        var primaryAssigned = false
        var start = -1
        fun closeSegment(endExclusive: Int) {
            if (start >= endExclusive) return
            val seg = samples.subList(start, endExclusive)
            if (seg.isEmpty()) return
            val isPrimary = !primaryAssigned
            primaryAssigned = true
            pieces.add(
                if (isPrimary) path.copy(samples = seg, modifiedAt = now)
                else path.copy(
        ...
        for (i in 0..samples.size) {
            val erased = i < samples.size && hit[i]
            when {
                start >= 0 && erased -> { closeSegment(i); start = -1 }
                start < 0 && !erased -> start = i
            }
        }
        if (start >= 0) closeSegment(samples.size)
        return pieces
    }
