...
fun DrawBox(
    ...
    Box(
        modifier = modifier
            ...
            .pointerInput(Unit) {
                // 只注册 onTap、不注册 onDoubleTap：detectTapGestures 在同时提供
                // 两个回调时会在 up 后等 doubleTapTimeout（300ms）确认无第二次
                // 点击才回调 onTap——TEXT/画笔/形状/橡皮模式下的点击会被吞掉
                // （尤其用户连点/双击时，第二次 tap 被判定为 double-tap，而
                // onDoubleTap 只在 SELECT 模式处理 → 两次点击全部无效）。移除后
                // tap 立即响应；SELECT 模式双击编辑改由宿主层实现——两次
                // SelectAt 命中同一已选中的 Text 即触发编辑（DrawBoxHost）。
                detectTapGestures(
                    onTap = { screenPos ->
                        val s = latestState
